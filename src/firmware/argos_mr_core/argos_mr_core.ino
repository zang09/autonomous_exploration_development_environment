...
ros::NodeHandle nh_;
ros::Subscriber<geometry_msgs::Twist> motor_vel_sub_("cmd_vel", veloCallback);

geometry_msgs::Twist motor_vel_;
ros::Publisher check_vel_pub_("mr_core/check_vel", &motor_vel_);
...
void funcParsing(char c)
{
  ...
  {
    pre_max_val_      = cur_max_val_;
    cur_max_val_      = 16;  //static v2:16, v3:12, v4: 20
...
void setBLDCMotorAuto()
{
  bldc1_val_ = ctr_val_ + (cur_max_val_ * auto_left_vel_);
  bldc2_val_ = ctr_val_ + (cur_max_val_ * auto_right_vel_);
  bldc3_val_ = ctr_val_ + (cur_max_val_ * auto_right_vel_);
  bldc4_val_ = ctr_val_ + (cur_max_val_ * auto_left_vel_);
...
{
  motor_vel_.linear.x = bldc1_val_; // left
  motor_vel_.linear.y = bldc2_val_; // rigtht
...
void writeBLDCMotor(bool mode)
{
  ...
  if (mode == true)
  {
    ...
    //Serial.print("Vel1: "); Serial.println(ctr_val_);
  }
  else
  {
    ...
    //Serial.print("Vel2: "); Serial.println(bldc1_val_);
  }
}
