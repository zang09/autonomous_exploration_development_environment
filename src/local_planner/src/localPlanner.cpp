...
double goalBound = 0.3;
double goalX = 0;
double goalY = 0;
std::vector<geometry_msgs::Point> goalList;
...
void goalArrayHandler(const rviz_flag_plugin::PointArray::ConstPtr& goalArray)
{
    goalList.clear();
    goalList.assign(goalArray->points.begin(), goalArray->points.end());

    if(goalList.empty()) return;

    if((goalOrigin.x != goalList.at(0).x) && (goalOrigin.y != goalList.at(0).y))
    {
        goalNum = 0;
    }

    goalOrigin = goalList.at(0);

    if(goalNum >= goalList.size())
    {
        getGoalPos = false;
    }
    else
    {
        getGoalPos = true;
        goalX = goalArray->points.at(goalNum).x;
        goalY = goalArray->points.at(goalNum).y;
    }
}
...
int main(int argc, char** argv)
{
    ...
    nhPrivate.getParam("goalBound", goalBound);
    ...
    ros::Subscriber subGoalArray = nh.subscribe<rviz_flag_plugin::PointArray>("/rviz/flag_points", 10, goalArrayHandler);
